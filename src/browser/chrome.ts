...
export async function launchOpenClawChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`[V] vclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`vclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ensureProfileCleanExit(userDataDir);
      ...
      log.warn(`vclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `[V] vclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
