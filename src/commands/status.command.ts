...
export async function statusCommand(
 ...
 const memoryValue = (() => {
  ...
  if (!memory) {
   ...
   // Custom (non-built-in) memory plugins cannot be probed, so show enabled.
 ...
 runtime.log(theme.heading("Vclaw status"));
 runtime.log("");
 ...
 if (pairingRecovery) {
  ...
  if (pairingRecovery.requestId) {
   runtime.log(
    theme.muted(
     `Recovery: ${formatCliCommand(`vclaw devices approve ${pairingRecovery.requestId}`)}`,
    ),
   );
  }
  runtime.log(theme.muted(`Fallback: ${formatCliCommand("vclaw devices approve --latest")}`));
  runtime.log(theme.muted(`Inspect: ${formatCliCommand("vclaw devices list")}`));
 }
 ...
 runtime.log(theme.muted(`Full report: ${formatCliCommand("vclaw security audit")}`));
 runtime.log(theme.muted(`Deep probe: ${formatCliCommand("vclaw security audit --deep")}`));
 ...
 runtime.log("");
 runtime.log(`FAQ: ${formatDocsLink("/faq", "docs.vclaw.ai/faq")}`);
 runtime.log(`Troubleshooting: ${formatDocsLink("/troubleshooting", "docs.vclaw.ai/troubleshooting")}`);
 runtime.log("");
 ...
 runtime.log(` Need to share?   ${formatCliCommand("vclaw status --all")}`);
 runtime.log(` Need to debug live? ${formatCliCommand("vclaw logs --follow")}`);
 if (gatewayReachable) {
  runtime.log(` Need to test Telegram? ${formatCliCommand("vclaw status --deep")}`);
 } else {
  runtime.log(` Fix reachability first: ${formatCliCommand("vclaw gateway probe")}`);
 }
}
