  ...
  if (!hasBonjourTool) {
    await prompter.note(
      [
        ...
        `Docs: ${formatDocsLink("/gateway/discovery", "docs.vclaw.ai/gateway/discovery")}`,
      ].join("\n"),
      "Discovery",
    );
  }
      ...
      if (mode === "direct") {
        ...
        await prompter.note(
          [
            ...
            `Docs: ${formatDocsLink("/gateway/remote", "docs.vclaw.ai/gateway/remote")}`,
          ].join("\n"),
          "SSH tunnel",
        );
      }
  ...
  if (authChoice === "token") {
    const selectedMode = await resolveSecretInputModeForEnvSelection({
      ...
      copy: {
        ...
        plaintextHint: "Stores the token directly in Vclaw config",
      },
    });
    if (selectedMode === "ref") {
    ...
    const selectedMode = await resolveSecretInputModeForEnvSelection({
      ...
      copy: {
        ...
        plaintextHint: "Stores the password directly in Vclaw config",
      },
    });
    if (selectedMode === "ref") {
      const resolved = await promptSecretRefForOnboarding({
