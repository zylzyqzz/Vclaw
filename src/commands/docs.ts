...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.vclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('vclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.vclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('vclaw docs "your query"')}`);
    }
    return;
  }
