...
export async function setupSkills(
  ...
  if (installable.length > 0) {
    const toInstall = await prompter.multiselect({
      ...
      options: [
        ...
        ...installable.map((skill) => ({
          value: skill.name,
          label: `${skill.emoji ?? "[skill]"} ${skill.name}`,
    ...
    for (const name of selected) {
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("vclaw doctor")}\` to review skills + requirements.`,
      );
      runtime.log(`Docs: ${formatDocsLink("/skills", "docs.vclaw.ai/skills")}`);
    }
  }
