...
export async function runSetupBootstrap(
  ...
  try {
    printBootstrapLogo();
    await prompter.intro("Vclaw minimal setup");
    ...
    if (!opts.skipTui) {
      ...
      if (result.message === "current-environment-not-suitable") {
        await prompter.outro(
          [
            "Setup complete",
            `Config: ${formatConfigPath(io.configPath)}`,
            `Workspace: ${shortenHomePath(workspace)}`,
            "",
            "Current environment is not suitable for auto-launching TUI",
            `Please run locally: ${formatCliCommand("vclaw")}`,
          ].join("\n"),
        );
        return;
      }
    ...
    if (process.platform === "linux") {
      ...
      if (loaded) {
        outroLines.push(
          "",
          "Gateway service is running in the background",
          `Status: ${formatCliCommand("vclaw status")}`,
          "Service will continue running after the terminal closes",
        );
      } else {
        outroLines.push(`Start: npm run start`);
      }
    } else {
      outroLines.push(`Start: npm run start`);
    }
    ...
    outroLines.push(
      `TUI: ${formatCliCommand("vclaw tui")}`,
      `Advanced: ${formatCliCommand("vclaw configure")}`,
    );
