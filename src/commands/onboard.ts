...
export async function onboardCommand(opts: OnboardOptions, runtime: RuntimeEnv = defaultRuntime) {
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    runtime.error(
      [
        ...
        `Read: ${formatDocsLink("/security", "docs.vclaw.ai/security")}`,
        `Re-run with: ${formatCliCommand("vclaw onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected. Vclaw runs best on WSL2.",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        `Guide: ${formatDocsLink("/windows", "docs.vclaw.ai/windows")}`,
      ].join("\n"),
    );
  }
