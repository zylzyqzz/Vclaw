...
async function runGatewayHealthCheck(params: {
  ...
  try {
    ...
    note(
      [
        "Docs:",
        formatDocsLink("/gateway/health", "docs.vclaw.ai/gateway/health"),
        formatDocsLink(
          "/gateway/troubleshooting",
          "docs.vclaw.ai/gateway/troubleshooting",
        ),
      ].join("\n"),
      "Health check help",
    );
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from the gateway config",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  note(
    [
      ...
      `Docs: ${formatDocsLink("/tools/web", "docs.vclaw.ai/tools/web")}`,
    ].join("\n"),
    "Web search",
  );
  ...
  if (enableSearch) {
    ...
    if (key || existingKey) {
      ...
      note(
        [
          "No key stored yet. web_search will not work until a key is available.",
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Vclaw update wizard" : "Vclaw configure");
        ...
        note(
          [
            ...
            `Docs: ${formatDocsLink(
              ...
              "docs.vclaw.ai/gateway/configuration",
            )}`,
          ].join("\n"),
          "Config issues",
        );
    ...
    const localUrl = "ws://127.0.0.1:19789";
    ...
    note(
      [
        ...
        gatewayStatusLine,
        `Terminal UI: ${formatCliCommand("vclaw tui")}`,
      ].join("\n"),
      "Vclaw runtime",
    );
