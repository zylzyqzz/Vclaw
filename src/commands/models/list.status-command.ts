...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("vclaw models auth setup-token")}\` or \`${formatCliCommand("vclaw configure")}\`.`
          : `Run \`${formatCliCommand("vclaw configure")}\` or set an API key env var.`;
