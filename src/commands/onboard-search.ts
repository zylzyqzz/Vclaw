...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with vclaw configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled - Vclaw will store a reference instead of the API key.",
  ...
  await prompter.note(
    [
      "No API key stored - web_search will not work until a key is available.",
