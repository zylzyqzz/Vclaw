      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["vclaw gateway run", "Run the gateway in the foreground."],
            ["vclaw gateway status", "Show service status and probe reachability."],
            ["vclaw gateway discover", "Find local and wide-area gateway beacons."],
            ["vclaw gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.vclaw.ai/cli/gateway")}\n`,
      ),
  );
