...
const SANDBOX_EXAMPLES = {
  main: [
    ["vclaw sandbox list", "List all sandbox containers."],
    ["vclaw sandbox list --browser", "List only browser containers."],
    ["vclaw sandbox recreate --all", "Recreate all containers."],
    ["vclaw sandbox recreate --session main", "Recreate a specific session."],
    ["vclaw sandbox recreate --agent mybot", "Recreate agent containers."],
    ["vclaw sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["vclaw sandbox list", "List all sandbox containers."],
    ["vclaw sandbox list --browser", "List only browser containers."],
    ["vclaw sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["vclaw sandbox recreate --all", "Recreate all containers."],
    ["vclaw sandbox recreate --session main", "Recreate a specific session."],
    ["vclaw sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["vclaw sandbox recreate --browser --all", "Recreate only browser containers."],
    ["vclaw sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["vclaw sandbox explain", "Show effective sandbox config."],
    ["vclaw sandbox explain --session agent:main:main", "Explain a specific session."],
    ["vclaw sandbox explain --agent work", "Explain an agent sandbox."],
    ["vclaw sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.vclaw.ai/cli/sandbox")}\n`,
    )
    .action(() => {
