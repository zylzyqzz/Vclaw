...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Vclaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["vclaw update", "Update a source checkout (git)"],
        ["vclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["vclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["vclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["vclaw update --dry-run", "Preview actions without changing anything"],
        ["vclaw update --no-restart", "Update without restarting the service"],
        ["vclaw update --json", "Output result as JSON"],
        ["vclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["vclaw update wizard", "Interactive update wizard"],
        ["vclaw --update", "Shorthand for vclaw update"],
      ] as const;
      const fmtExamples = examples
        .map(([cmd, desc]) => `  ${theme.command(formatCliCommand(cmd))} ${theme.muted(`# ${desc}`)}`)
        .join("\n");
      return `
...
${theme.heading("Switch channels:")}
  ...
  - Run ${formatCliCommand("vclaw update status")} to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.vclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      "after",
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vclaw update status", "Show channel + version status."],
          ["vclaw update status --json", "JSON output."],
          ["vclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.vclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
