...
const DEFAULT_COMPLETION_BIN = "vclaw";
const COMPLETION_PROFILE_HEADERS = new Set(["# Vclaw Completion", "# WeiClaw Completion"]);
const COMPLETION_COMMAND_RE = /\b(?:openclaw|weiclaw|vclaw)\s+completion\b/;
...
export async function completionCacheExists(
  ...
  binName = DEFAULT_COMPLETION_BIN,
): Promise<boolean> {
...
function isCompletionProfileHeader(line: string): boolean {
  return COMPLETION_PROFILE_HEADERS.has(line.trim());
}

function isCompletionProfileLine(line: string, binName: string, cachePath: string | null): boolean {
  if (line.includes(`${binName} completion`) || COMPLETION_COMMAND_RE.test(line)) {
    return true;
  }
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  ...
  return (
    ...
    line.includes("<(weiclaw completion") ||
    line.includes("<(vclaw completion") ||
    (COMPLETION_COMMAND_RE.test(line) && line.includes("| source"))
  );
}
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.vclaw.ai/cli/completion")}\n`,
    )
    .addOption(
...
export async function installCompletion(shell: string, yes: boolean, binName = DEFAULT_COMPLETION_BIN) {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
