...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall Vclaw and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            "- Pin Vclaw Browser Relay, then click it on the target tab (badge ON)",
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.vclaw.ai/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
        ...
        defaultRuntime.error(
          danger(
            [
              ...
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.vclaw.ai/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
