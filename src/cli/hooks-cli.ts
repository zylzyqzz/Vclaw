  ...
  if (hasRequirements) {
    ...
    if (hook.requirements.bins.length > 0) {
      const binsStatus = hook.requirements.bins.map((bin) => {
        ...
        return missing ? theme.error(`[missing] ${bin}`) : theme.success(`[ready] ${bin}`);
      });
    ...
    if (hook.requirements.anyBins.length > 0) {
      const anyBinsStatus =
        hook.missing.anyBins.length > 0
          ? theme.error(`[missing] (any of: ${hook.requirements.anyBins.join(", ")})`)
          : theme.success(`[ready] (any of: ${hook.requirements.anyBins.join(", ")})`);
    ...
    if (hook.requirements.env.length > 0) {
      const envStatus = hook.requirements.env.map((env) => {
        ...
        return missing ? theme.error(`[missing] ${env}`) : theme.success(`[ready] ${env}`);
      });
      lines.push(`${theme.muted("  Environment:")} ${envStatus.join(", ")}`);
    }
    if (hook.requirements.config.length > 0) {
      const configStatus = hook.configChecks.map((check) => {
        return check.satisfied ? theme.success(`[ready] ${check.path}`) : theme.error(`[missing] ${check.path}`);
      });
    ...
    if (hook.requirements.os.length > 0) {
      const osStatus =
        hook.missing.os.length > 0
          ? theme.error(`[missing] (${hook.requirements.os.join(", ")})`)
          : theme.success(`[ready] (${hook.requirements.os.join(", ")})`);
...
export function formatHooksCheck(report: HookStatusReport, opts: HooksCheckOptions): string {
  ...
  if (notEligible.length > 0) {
    ...
    for (const hook of notEligible) {
      ...
      lines.push(`  ${hook.emoji ?? "[hook]"} ${hook.name} - ${reasons.join("; ")}`);
    }
  }
...
export async function enableHook(hookName: string): Promise<void> {
  ...
  defaultRuntime.log(
    `${theme.success("OK")} Enabled hook: ${hook.emoji ?? "[hook]"} ${theme.command(hookName)}`,
  );
}
...
export async function disableHook(hookName: string): Promise<void> {
  ...
  await writeConfigFile(nextConfig);
  defaultRuntime.log(
    `${theme.warn("OFF")} Disabled hook: ${hook.emoji ?? "[hook]"} ${theme.command(hookName)}`,
  );
}
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.vclaw.ai/cli/hooks")}\n`,
    );
