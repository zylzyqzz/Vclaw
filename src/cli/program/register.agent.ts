...
${formatHelpExamples([
  ['vclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['vclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'vclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'vclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['vclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'vclaw agent --agent ops --message "Generate report" --deliver --reply-channel telegram --reply-to "@ops_reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.vclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.vclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['vclaw agents set-identity --agent main --name "Vclaw" --emoji "V"', "Set name + icon."],
  ["vclaw agents set-identity --agent main --avatar avatars/vclaw.png", "Set avatar path."],
  [
    "vclaw agents set-identity --workspace ~/.vclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "vclaw agents set-identity --identity-file ~/.vclaw/workspace/IDENTITY.md --agent main",
