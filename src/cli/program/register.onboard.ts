...
export function registerOnboardCommand(program: Command) {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.vclaw.ai/cli/onboard")}\n`,
    )
    .option("--workspace <dir>", "Agent workspace directory (default: ~/.vclaw/workspace)")
    .option(
      "--reset",
  ...
  command
    ...
    .option("--skip-ui", "Skip terminal launch prompt (legacy flag name)")
