    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "vclaw node run --host 127.0.0.1 --port 19789",
            "Run the node host in the foreground.",
          ],
          ["vclaw node status", "Check node host service status."],
          ["vclaw node install", "Install the node host service."],
          ["vclaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.vclaw.ai/cli/node")}\n`,
    );
